...
> [Master identity prompt] Create a [portrait / candid / lifestyle] photograph of her [action] in [setting]. Wardrobe: [outfit and colors]. Expression: [expression]. Camera: [shot size, angle, lens feel]. Lighting: [description]. Composition for [platform and aspect ratio], with [space for caption if needed]. Keep her appearance consistent with all references. [Negative instructions]

### Remi scene library

| Use | Scene prompt (insert into template) | Aspect ratio |
|---|---|---|
| Profile photo | Chest-up portrait, big genuine smile, looking at camera, cornflower-blue sleeveless top, fine necklace, soft plain light background, soft daylight | 1:1 |
| Level-Up Monday cover | Waist-up at a bright desk with a laptop, pointing toward open space beside her, excited expression, white blouse, soft daylight | 4:5 (IG/FB), 9:16 (Stories/TikTok) |
...
> Use the supplied images to maintain the same adult woman's face, hairline, eye and brow relationship, nose, and smile through the entire clip. [Duration] seconds, [aspect ratio], [setting and action], [wardrobe], [camera movement]. Keep motion subtle and physically natural, with stable facial structure between frames and consistent lighting. [Speech: see below.] No child, no other person, no on-screen text. Generate a clean take without captions; add reviewed captions during editing.
...
Remi's *words* come from her scripts and `04-voice-and-style.md`. Whether Remi *speaks on camera* with this face is a separate decision:

- **Until she approves a voice:** use silent clips with captions, or a clearly separate voiceover that isn't lip-synced to her face.
- **If she approves a generated voice:** use a voice sample she knowingly supplied for this purpose, confirm the tool's rules on voice cloning, and record her approval in `07-likeness-consent.md`.
- **Never** script Remi to say something presented as the real woman's own memory, opinion, or experience.

## Review checklist (before any post)

- [ ] She recognizes herself in the face, especially the smile and eyes.
- [ ] Face and hair are consistent from frame to frame.
- [ ] No child or other real person appears anywhere.
- [ ] Nothing could be mistaken for a real event or a real statement by her.
- [ ] Post is labeled as AI-generated where the platform requires it, and the account bio says Remi is AI.
- [ ] Any brand, product, or sponsorship is approved by her and disclosed.
- [ ] She approved this image or clip and the post it goes with (if final approval per post is required; see `07-likeness-consent.md`).
- [ ] Prompt, output, and approval logged in the creation record in `05-visual-identity.md`.
